...
public final class DeepAgentExample {
    ...
    public static void main(String[] arguments) {
        ...
        try (DeepAgent agent = DeepAgent.builder(new ScriptedModel())
                ...
                .build()) {
            DeepRun paused = agent.invoke("demo", "Calculate the confidence score and save a finding in notes/finding.txt").toCompletableFuture().join();
    ...
    private static final class ScriptedModel implements ChatModel {
        ...
        @Override public java.util.concurrent.CompletionStage<ChatResponse> complete(io.cortavyn.model.api.ChatRequest request) {
            calls++;
            ChatMessage message = switch (calls) {
                case 1 -> new ChatMessage(ChatMessageRole.ASSISTANT, "", List.of(), null,
                        List.of(new ToolCall("calculate-confidence", "eval", Map.of("code", "0.8 + 0.15"))), Map.of());
                case 2 -> new ChatMessage(ChatMessageRole.ASSISTANT, "", List.of(), null,
                        List.of(new ToolCall("write-finding", "write_file", Map.of("path", "notes/finding.txt", "content", "Durable state keeps the research trail."))), Map.of());
                default -> new ChatMessage(ChatMessageRole.ASSISTANT, "Finding saved.");
            };
